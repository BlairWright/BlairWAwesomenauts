game.SpendGold = Object.extend({
    ...
    setBuyText: function(){
      game.data.buytext = new (me.Renderable.extend({
                    ...
                    draw: function(renderer){
                        this.font.draw(renderer.getContext(), "PRESS F1-F6 TO BUY, B TO EXIT. Current Gold: " + game.data, this.pos.x, this.pos.y);
    ...
    makePurchase: function(skill){
        if(skill === 1){
            game.data.glod -= ((game.data.skill1 +1)* 10);
            game.data.skill1 += 1;
            game.data.playerAttack += 1;
        }else if(skill === 2){
            game.data.glod -= ((game.data.skill2 +1)* 10);
            game.data.skill2 += 1;
        }else if(skill === 3){
            game.data.glod -= ((game.data.skill3 +1)* 10);
            game.data.skill3 += 1;
        }else if(skill === 4){
            game.data.glod -= ((game.data.ability1 +1)* 10);
            game.data.ability1 += 1;
        }else if(skill === 5){
            game.data.glod -= ((game.data.ability2 +1)* 10);
            game.data.ability2 += 1;
        }else if(skill === 6){
           game.data.glod -= ((game.data.ability3 +1)* 10);
